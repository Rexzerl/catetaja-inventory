...
<head>
    ...
    <title>Edit Barang - CatetAja!</title>
...
<body class="bg-surface dark:bg-slate-900 text-on-surface dark:text-white transition-colors duration-300 min-h-screen">
    ...
    <nav class="bg-surface dark:bg-slate-800 border-b border-outline dark:border-slate-700 fixed top-0 w-full z-50 h-16 flex justify-between items-center px-4 md:px-margin-desktop">
        ...
        <div class="flex items-center gap-6">
            ...
            <div class="relative group">
                ...
                <!-- JEMBATAN TRANSPARAN & DROPDOWN MENU -->
    ...
    <main class="pt-24 pb-20 md:pb-12 px-4 md:px-margin-desktop md:ml-64">
        ...
        <div class="w-full max-w-4xl mx-auto">
            ...
            <header class="mb-8 flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-black uppercase tracking-tight text-ts-red dark:text-white leading-none">Ubah Data Barang</h1>
                </div>
            </header>
            ...
            <section class="bg-white dark:bg-slate-800 border border-black dark:border-slate-600 hard-shadow p-6 md:p-8">
                ...
                <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant dark:text-slate-400">Ganti Foto Barang (Opsional)</label>
                        <div id="drop-area" class="relative w-full h-48 bg-surface-container-low dark:bg-slate-900 border-2 border-dashed border-black dark:border-slate-500 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors overflow-hidden group">
                            ...
                            <img id="image-preview" src="{{ $product->image ? asset('storage/' . $product->image) : '' }}" class="absolute inset-0 w-full h-full object-contain {{ $product->image ? '' : 'hidden' }} bg-white dark:bg-slate-900 z-10" alt="Preview">
                            
                            <div id="upload-content" class="flex flex-col items-center text-on-surface-variant dark:text-slate-400 {{ $product->image ? 'hidden' : '' }}">
                                <span class="material-symbols-outlined text-4xl mb-2">cloud_upload</span>
                                <span class="font-bold text-sm">Drag & Drop foto baru di sini</span>
                                <span class="text-xs mt-1">atau klik untuk mengganti (JPG, PNG)</span>
                            </div>
                            
                            <input type="file" id="image" name="image" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20">
                        </div>
                        
                        <button type="button" id="remove-image-btn" class="{{ $product->image ? '' : 'hidden' }} text-ts-red text-xs font-bold uppercase mt-2 hover:underline flex items-center gap-1">
                            <span class="material-symbols-outlined text-[14px]">delete</span> Batal / Ganti Foto
                        </button>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input class="w-full px-4 py-3 bg-surface-container-low dark:bg-slate-900 border-2 border-black dark:border-slate-600 font-mono focus:ring-0 focus:border-ts-red outline-none dark:text-white" name="product_code" value="{{ old('product_code', $product->product_code) }}" required type="text">
                        </div>
                        <div class="space-y-2">
                            ...
                            <input class="w-full px-4 py-3 bg-surface-container-low dark:bg-slate-900 border-2 border-black dark:border-slate-600 focus:ring-0 focus:border-ts-red outline-none dark:text-white" name="name" value="{{ old('name', $product->name) }}" required type="text">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <select class="w-full px-4 py-3 bg-surface-container-low dark:bg-slate-900 border-2 border-black dark:border-slate-600 focus:ring-0 focus:border-ts-red outline-none dark:text-white" name="category_id" required>
                        ...
                        <div class="space-y-2">
                            ...
                            <select class="w-full px-4 py-3 bg-surface-container-low dark:bg-slate-900 border-2 border-black dark:border-slate-600 focus:ring-0 focus:border-ts-red outline-none dark:text-white" name="condition" required>
                                <option value="Bagus" {{ old('condition', $product->condition) === 'Bagus' ? 'selected' : '' }}>Bagus (Siap Pakai)</option>
                                <option value="Rusak Ringan" {{ old('condition', $product->condition) === 'Rusak Ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                                <option value="Perbaikan" {{ old('condition', $product->condition) === 'Perbaikan' ? 'selected' : '' }}>Dalam Perbaikan</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input class="w-full px-4 py-3 bg-surface-container-low dark:bg-slate-900 border-2 border-black dark:border-slate-600 focus:ring-0 focus:border-ts-red outline-none dark:text-white" name="location" value="{{ old('location', $product->location) }}" required type="text">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant dark:text-slate-400">Stok (Unit)</label>
                            <input class="w-full px-4 py-3 bg-surface-container-low dark:bg-slate-900 border-2 border-black dark:border-slate-600 focus:ring-0 focus:border-ts-red outline-none dark:text-white" name="stock" value="{{ old('stock', $product->stock) }}" min="0" required type="number">
                        </div>
                    </div>

                    <button type="submit" class="w-full py-4 bg-ts-orange hover:bg-orange-500 text-black font-bold uppercase border border-black hard-shadow hover:scale-[1.01] active:scale-95 transition-all flex items-center justify-center gap-2 mt-4">
                        Update Data Barang <span class="material-symbols-outlined text-[20px]">update</span>
                    </button>
                </form>
            </section>
    ...
    <script>
        ...
        removeBtn.addEventListener('click', () => {
            fileInput.value = ''; 
            const hasOldImage = "{{ $product->image ? 'true' : 'false' }}";
